...
public class StaticPackets implements IStaticPackets {
    // C2S
    // Chat Message
//    public CPacketChatMessage createCPacketChatMessage(String message) {
//        return new CPacketChatMessageMesh(new ChatMessageC2SPacket(message));
//    }

    @Override
    public CPacketChatMessage createCPacketChatMessage(ByteBuf buf) {
        return new CPacketChatMessageMesh(new ChatMessageC2SPacket(new PacketByteBuf(buf)));
    }

    // Confirm Teleport
    @Override
    public CPacketConfirmTeleport createCPacketConfirmTeleport(int id) {
        return new CPacketConfirmTeleportMesh(new TeleportConfirmC2SPacket(id));
    }

    @Override
    public CPacketConfirmTeleport createCPacketConfirmTeleport(ByteBuf buf) {
        return new CPacketConfirmTeleportMesh(new TeleportConfirmC2SPacket(new PacketByteBuf(buf)));
    }

    // Hand Swing
    @Override
    public CPacketHandSwing createCPacketHandSwing(Hand hand) {
        return new CPacketHandSwingMesh(new HandSwingC2SPacket(MCEnum.hand(hand)));
    }

    @Override
    public CPacketHandSwing createCPacketHandSwing(ByteBuf buf) {
        return new CPacketHandSwingMesh(new HandSwingC2SPacket(new PacketByteBuf(buf)));
    }

    // Input
    @Override
    public CPacketInput createCPacketInput(float sideways, float forward, boolean jumping, boolean sneaking) {
        return new CPacketInputMesh(new PlayerInputC2SPacket(sideways, forward, jumping, sneaking));
    }

    @Override
    public CPacketInput createCPacketInput(ByteBuf buf) {
        return new CPacketInputMesh(new PlayerInputC2SPacket(new PacketByteBuf(buf)));
    }

    // Movement
    @Override
    public CPacketMovePlayer.OnGround createCPacketMovePlayerOnGround(boolean onGround) {
        return new CPacketMovePlayerMesh.OnGround<>(new PlayerMoveC2SPacket.OnGroundOnly(onGround));
    }

    @Override
    public CPacketMovePlayer.OnGround createCPacketMovePlayerOnGround(ByteBuf buf) {
        return new CPacketMovePlayerMesh.OnGround<>(PlayerMoveC2SPacket.OnGroundOnly.read(new PacketByteBuf(buf)));
    }

    @Override
    public CPacketMovePlayer.Position createCPacketMovePlayerPosition(double x, double y, double z, boolean onGround) {
        return new CPacketMovePlayerMesh.Position(new PlayerMoveC2SPacket.PositionAndOnGround(x, y, z, onGround));
    }

    @Override
    public CPacketMovePlayer.Position createCPacketMovePlayerPosition(ByteBuf buf) {
        return new CPacketMovePlayerMesh.Position(PlayerMoveC2SPacket.PositionAndOnGround.read(new PacketByteBuf(buf)));
    }

    @Override
    public CPacketMovePlayer.Rotation createCPacketMovePlayerRotation(float yaw, float pitch, boolean onGround) {
        return new CPacketMovePlayerMesh.Rotation(new PlayerMoveC2SPacket.LookAndOnGround(yaw, pitch, onGround));
    }

    @Override
    public CPacketMovePlayer.Rotation createCPacketMovePlayerRotation(ByteBuf buf) {
        return new CPacketMovePlayerMesh.Rotation(PlayerMoveC2SPacket.LookAndOnGround.read(new PacketByteBuf(buf)));
    }

    @Override
    public CPacketMovePlayer.PositionRotation createCPacketMovePlayerPositionRotation(double x, double y, double z, float yaw, float pitch, boolean onGround) {
        return new CPacketMovePlayerMesh.PositionRotation(new PlayerMoveC2SPacket.Full(x, y, z, yaw, pitch, onGround));
    }

    @Override
    public CPacketMovePlayer.PositionRotation createCPacketMovePlayerPositionRotation(ByteBuf buf) {
        return new CPacketMovePlayerMesh.PositionRotation(PlayerMoveC2SPacket.Full.read(new PacketByteBuf(buf)));
    }

    // Move Vehicle
    @Override
    public CPacketMoveVehicle createCPacketMoveVehicle(Entity entity) {
        return new CPacketMoveVehicleMesh(new VehicleMoveC2SPacket((net.minecraft.entity.Entity) entity));
    }

    @Override
    public CPacketMoveVehicle createCPacketMoveVehicle(double x, double y, double z, float yaw, float pitch) {
        // kind of a hack
    ...
    @Override
    public CPacketMoveVehicle createCPacketMoveVehicle(ByteBuf buf) {
        return new CPacketMoveVehicleMesh(new VehicleMoveC2SPacket(new PacketByteBuf(buf)));
    }

    // Steer Boat
    @Override
    public CPacketSteerBoat createCPacketSteerBoat(boolean left, boolean right) {
        return new CPacketSteerBoatMesh(new BoatPaddleStateC2SPacket(left, right));
    }

    @Override
    public CPacketSteerBoat createCPacketSteerBoat(ByteBuf buf) {
        return new CPacketSteerBoatMesh(new BoatPaddleStateC2SPacket(new PacketByteBuf(buf)));
    }

    // Use Block
    @Override
    public CPacketUseBlock createCPacketUseBlock(Hand hand, BlockPos blockPos, Facing facing, Vec3d vector, boolean insideBlock, int sequence) {
    ...
    @Override
    public CPacketUseBlock createCPacketUseBlock(ByteBuf buf) {
        return new CPacketUseBlockMesh(new PlayerInteractBlockC2SPacket(new PacketByteBuf(buf)));
    }

    // Use Entity
    @Override
    public CPacketUseEntity createCPacketUseEntityAttack(Entity entity, boolean sneaking) {
        return new CPacketUseEntityMesh(PlayerInteractEntityC2SPacket.attack((net.minecraft.entity.Entity) entity, sneaking));
    }

    @Override
    public CPacketUseEntity createCPacketUseEntityInteract(Entity entity, Hand hand, boolean sneaking) {
    ...
    @Override
    public CPacketUseEntity createCPacketUseEntityInteractAt(Entity entity, Hand hand, Vec3d pos, boolean sneaking) {
    ...
    @Override
    public CPacketUseEntity createCPacketUseEntity(ByteBuf buf) {
        return new CPacketUseEntityMesh(new PlayerInteractEntityC2SPacket(new PacketByteBuf(buf)));
    }

    // Use Item
    @Override
    public CPacketUseItem createCPacketUseItem(Hand hand, int sequence) {
        return new CPacketUseItemMesh(new PlayerInteractItemC2SPacket(MCEnum.hand(hand), sequence));
    }

    @Override
    public CPacketUseItem createCPacketUseItem(ByteBuf buf) {
    ...
    // Advancement Update
    @Override
    public SPacketAdvancementUpdate createSPacketAdvancementUpdate(boolean isFirstSync, Collection<Advancement> toAdd, Set<LocationIdentifier> toRemove, Map<LocationIdentifier, AdvancementProgress> progressUpdates) {
    ...
    @Override
    public SPacketAdvancementUpdate createSPacketAdvancementUpdate(ByteBuf buf) {
        return new SPacketAdvancementUpdateMesh(new AdvancementUpdateS2CPacket(new PacketByteBuf(buf)));
    }

    // Block Action
    @Override
    public SPacketBlockAction createSPacketBlockAction(BlockPos blockPos, Block block, @Nullable Enum<?> blockVariant, int instrument, int pitch) {
...
//        return new SPacketBlockActionMesh(new BlockEventS2CPacket((net.minecraft.util.math.BlockPos) blockPos, block, instrument, pitch));
    ...
    @Override
    public SPacketBlockAction createSPacketBlockAction(ByteBuf buf) {
        return new SPacketBlockActionMesh(new BlockEventS2CPacket(new PacketByteBuf(buf)));
    }

    // Block Update
    @Override
    public SPacketBlockUpdate createSPacketBlockUpdate(BlockPos blockPos, BlockState<?> blockState) {
    ...
    @Override
    public SPacketBlockUpdate createSPacketBlockUpdate(ByteBuf buf) {
        return new SPacketBlockUpdateMesh(new BlockUpdateS2CPacket(new PacketByteBuf(buf)));
    }

    // Break Progress
    @Override
    public SPacketBreakProgress createSPacketBreakProgress(int entityId, BlockPos blockPos, int progress) {
    ...
    @Override
    public SPacketBreakProgress createSPacketBreakProgress(ByteBuf buf) {
        return new SPacketBreakProgressMesh(new BlockBreakingProgressS2CPacket(new PacketByteBuf(buf)));
    }

    // Player Position Rotation
    @Override
    public SPacketPlayerPositionRotation createSPacketPlayerPositionRotation(double x, double y, double z, float yaw, float pitch, Set<SPacketPlayerPositionRotation.Flag> flags, int teleportId, boolean shouldDismount) {
    ...
    @Override
    public SPacketPlayerPositionRotation createSPacketPlayerPositionRotation(ByteBuf buf) {
...
//    public SPacketTileEntityUpdate createSPacketTileEntityUpdate(BlockPos blockPos) {
//        throw new RuntimeException();
//    }

    @Override
    public SPacketTileEntityUpdate createSPacketTileEntityUpdate(ByteBuf buf) {
